<!DOCTYPE html>
<html>
<head>
    <?php include "parts/meta.php"; ?>
</head>
<body>

<div class="banner">
    <img src="img/banner.jpg"/>
    <div class="banner_mark">
        Home
    </div>
</div>
<div class="con1">
    <div class="con1_con">
        <h2>Table of Contents</h2>
        <div class="titles_con">
            <div><a href="#shop">Typography</a></div>
            <div><a href="#paragraphs">Paragraphs</a></div>
            <div><a href="#cards">Cards</a></div>
            <div><a href="#Color">Color</a></div>
            <div><a href="#View">View Window</a></div>
            <div><a href="#Navigations">Navigations</a></div>
            <div><a href="#Tables">Tables</a></div>
            <div><a href="">Forms</a></div>
        </div>
        

<div id="shop" class="shop">
    <div class="shop_title">Typography</div>
    <div class="shop_con">
        <div>
            <img src="img/img1.jpg"/>
            <div class="shop_name">
                SHOP ONE
            </div>
            <div class="shop_price">
                30$
            </div>
            <button class="buy_btn">BUY</button>
        </div>
        <div>
            <img src="img/img2.jpg"/>
            <div class="shop_name">
                SHOP TWO
            </div>
            <div class="shop_price">
                29.99$
            </div>
            <button class="buy_btn">BUY</button>
        </div>
        <div>
            <img src="img/img3.jpg"/>
            <div class="shop_name">
                SHOP THREE
            </div>
            <div class="shop_price">
                30.99$
            </div>
            <button class="buy_btn">BUY</button>
        </div>
        <div>
            <img src="img/img4.jpg"/>
            <div class="shop_name">
                SHOP FOUR
            </div>
            <div class="shop_price">
                31.99$
            </div>
            <button class="buy_btn">BUY</button>
        </div>
    </div>
</div>
<div id="paragraphs" class="paragraphs">
    <div class="shop_title">Paragraphs</div>
    <div class="paragraphs_text">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus nisi quaerat corporis doloribus dolorem repellendus
        sunt error,
        numquam officiis vitae asperiores delectus magni aperiam reiciendis soluta magnam! Exercitationem, voluptatibus doloremque!
    </div>
    <div class="paragraphs_text">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus nisi quaerat corporis doloribus dolorem repellendus
        sunt error,
        numquam officiis vitae asperiores delectus magni aperiam reiciendis soluta magnam! Exercitationem, voluptatibus doloremque!
    </div>
    <div class="paragraphs_text">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus nisi quaerat corporis doloribus dolorem repellendus
        sunt error,
        numquam officiis vitae asperiores delectus magni aperiam reiciendis soluta magnam! Exercitationem, voluptatibus doloremque!
    </div>
</div>
<div id="cards" class="Cards">
    <div class="shop_title">Card</div>
    <div class="card_con">
        <div class="card1">
            Card
        </div>
        <div class="card2">
            Card
        </div>
        <div class="card3">
            Card
        </div>
    </div>
</div>
<div id="Color" class="color">
    <div class="shop_title">Color</div>
    <div class="color_con">
        <div style="background-color: #222222">#222222</div>
        <div style="background-color: #FF4500">#FF4500</div>
        <div style="background-color: #FF7F50">#FF7F50</div>
        <div style="background-color: #EEEDED;border: 1px solid black;">#EEEDED</div>
        <div style="background-color: #593D41">#593D41</div>
    </div>
</div>
<div class="bg_title" id="View">
    <div class="bg_title_con">View Window</div>
</div>
<div class="flex_img"><img src="img/flex.jpg"/></div>
<div class="header" id="Navigations">
    <div class="shop_title">Navigations</div>
    <div class="header_con">
    <nav class="navbar navbar-default" role="navigation">
        <div class="container-fluid">
            <div class="navbar-header">
                <button type="button" class="navbar-toggle" data-toggle="collapse"
                        data-target="#example-navbar-collapse">
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                </button>
                <a class="navbar-brand" href="#"><h4>Style guide</h4></a>
            </div>
            <div class="collapse navbar-collapse" id="example-navbar-collapse">
                <ul class="nav navbar-nav" style="float: right">
                    <li class=""><a href="#"><h4>HOME</h4></a></li>
                    <li><a href="#"><h4>SHOP</h4></a></li>
                    <li><a href="#"><h4>NEWS</h4></a></li>
                    <li><a href="#"><h4>ABOUT</h4></a></li>
                </ul>
            </div>
        </div>
    </nav>
    </div>
</div>
<div class="shop_table" id="Tables">
    <div class="shop_title">Tables</div>
    <div class="table_con">
        <h2>Basic Table</h2>
        <table class="shop_tables">
            <tr>
                <th>ID</th>
                <th>Office</th>
                <th>Name</th>
                <th>Email</th>
                <th>Phone</th>
            </tr>
            <tr>
                <td>1</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>2</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>3</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>4</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>5</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
        </table>
    </div>
    <div class="table_con">
        <h2>Striped Table</h2>
        <table class="shop_tables">
            <tr>
                <th>ID</th>
                <th>Office</th>
                <th>Name</th>
                <th>Email</th>
                <th>Phone</th>
            </tr>
            <tr style="background-color: #fff">
                <td>1</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>2</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr style="background-color: #fff">
                <td>3</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>4</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr style="background-color: #fff">
                <td>5</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
        </table>
    </div>
    <div class="table_con">
        <h2>Striped Lined Vertical</h2>
        <table class="shop_tables" style="border: 2px solid #fff">
            <tr>
                <th>ID</th>
                <th>Office</th>
                <th>Name</th>
                <th>Email</th>
                <th>Phone</th>
            </tr>
            <tr>
                <td>1</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>2</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>3</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>4</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>5</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
        </table>
    </div>
    <div class="table_con">
        <h2>Striped Lined All</h2>
        <table class="shop_tables shop_tables4" border="1">
            <tr>
                <th>ID</th>
                <th>Office</th>
                <th>Name</th>
                <th>Email</th>
                <th>Phone</th>
            </tr>
            <tr>
                <td>1</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>2</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>3</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>4</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>5</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
        </table>
    </div>
    <div class="table_con">
        <h2>Striped Table</h2>
        <table class="shop_tables" border="1">
            <tr>
                <th>ID</th>
                <th>Office</th>
                <th>Name</th>
                <th>Email</th>
                <th>Phone</th>
            </tr>
            <tr style="background-color: #fff">
                <td>1</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>2</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr style="background-color: #fff">
                <td>3</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr>
                <td>4</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
            <tr style="background-color: #fff">
                <td>5</td>
                <td>156</td>
                <td>Danny</td>
                <td>andy135@eliohung.com</td>
                <td>415-3211</td>
            </tr>
        </table>
    </div>
</div>
<div class="forms" id="Forms">
    <div class="shop_title">Forms</div>
    <div class="forms_title">default form</div>
    <div class="forms_con">
        <input type="text" placeholder="name">
        <br>
        <br>
        <input type="password" placeholder="password">
        <br>
        <br>
        <input type="email" placeholder="email">
        <br>
        <br>
        <select>
            <option>java</option>
            <option>c#</option>
            <option>html</option>
            <option>css</option>
        </select>
        <br>
        <br>
        <button>button</button>
        <br>
        <br>
        <label>Male :</label> <input type="checkbox">
        <br>
        <br>
        <label>Male :</label><input type="checkbox">
        <br>
        <br>
        <label>Male :</label><input type="radio">
        <br>
        <br>
        <label>Male :</label><input type="radio">
        <br>
        <br>

    </div>

    <div class="forms_title">Contact Form</div>
    <div class="forms_con">
        <div>
            <label>Name:</label>
            <br>
            <input type="text" placeholder="Name" class="ipt_name">
        </div>
        <div>
            <label>Password:</label>
            <br>
            <input type="password" placeholder="Password" class="ipt_name">
        </div>
        <div>
            <label>Email:</label>
            <br>
            <input type="email" placeholder="Password" class="ipt_name">
        </div>
        <div>
            <label>Choose:</label>
            <br>
            <select class="ipt_name">
                <option>java</option>
                <option>c#</option>
                <option>html</option>
                <option>css</option>
            </select>
        </div>
        <textarea class="textarea" placeholder="content" style="margin-bottom: 20px"></textarea>
        <div>
            <label>Male1 :</label> <input type="checkbox" style="margin-right: 20px">
            <label>Male2 :</label> <input type="checkbox" style="margin-right: 20px">
            <label>Male3 :</label><input type="radio" style="margin-right: 20px">
            <label>Male4 :</label><input type="radio" style="margin-right: 20px">
        </div>
        <button type="button" class="btn_sub">Submit</button>
    </div>

    <?php include "parts/footer.php"; ?>
</div>
</body>
</html>
